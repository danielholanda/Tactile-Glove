...
//qq95538 import posix standard time.h
#include <time.h>
#include <unistd.h>
#include "edison_setting.h"
/*qq95538 avoid msp430 clock definitions.
#include "stdio.h"
#include "msp430.h"
...
*/
//qq95538 This line is a specific #pragma command for msp430 to manipulate time interrupt.
//#pragma vector=TIMERB0_VECTOR
/*qq95538 This function also sets time interrupt.
...
*/
/*qq95538 Intel Edison does not involve an clock enable/disable option.
...
*/
/* qq95538 As a standard posix system, Edison has Time.h supporting, does not need an initiative function.
...
/*qq95538 Edison does not support these clock frequency operations.
...
int edison_get_clock_ms(unsigned long *count)
{

	if (!count)
        return 1;
    //qq95538 msp430 aquire clock.timestamp value as a time.
	//count[0] = clock.timestamp;
	clock_t cpu_clock_count_stamp;
	cpu_clock_count_stamp = clock();
	if (cpu_clock_count_stamp == -1){
		fprintf(stderr, "time.h clock() function failed, can't get time stamp");
		return -1;
	}
	count[0] = (cpu_clock_count_stamp * 1000) / CLOCKS_PER_SEC;
    return 0;
}

inline int edison_delay_ms(unsigned long num_ms)
{
    /*
    ...
    while (clock.timestamp - start_time < num_ms)
  	...
  	return 0;
  	*/
	//There is no description of param or return value from int usleep(__useconds_t) in unistd.h
	return usleep(num_ms * 1000);


}

/*qq95538 What is the meaning, time slow, clock ticks operate or interrupt delay?
...
/*qq95538 It seems that time interrupt callback is not used in eMPL.
